/* Axel Alvarez Lab #10*/
//Purpose: create adjacency list graph and traverse via BFS
...
void graph::BFS(int initial_state){
    //creating an array of bool True or False Values 
    vst_vertice = new bool[data];
    for(int i = 0; i < data ; i++ )
        vst_vertice[i] = false;
    list<int> queue;
    //checking if we visited initial state of the directed graph
    vst_vertice[initial_state] = true;
    queue.push_back(initial_state);
    //creating list iterorator it
    list<int>::iterator it;
    //while queue is not empty traverse queue
    while(!queue.empty()){
